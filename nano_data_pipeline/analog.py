...
def build_packing_isolation_preservation_mix_dataset(
    feedback_manifest_path: Path,
    failure_family_receipt_path: Path,
    base_dataset_path: Path,
    broad_dataset_path: Path,
    ...
    if (
        receipt.get("schema_version")
        != "nano_harness_failure_family_receipt_v1"
        or "packing_efficiency_effective_volume"
        not in {row["family"] for row in receipt.get("families", [])}
        or receipt.get("policy", {}).get("contains_case_ids") is not False
        or receipt.get("policy", {}).get("contains_prompts") is not False
        or receipt.get("policy", {}).get("contains_references") is not False
        ...
        raise ValueError("failure-family receipt violates the v9 boundary")
    ...
    validate_analog_dataset(broad)
    if base.get("dataset_id") != "targeted-preservation-mix-v6":
        raise ValueError("packing-isolation base must be v6")
    if broad.get("dataset_id") != "failure-targeted-preservation-mix-v7":
        raise ValueError("packing-isolation source must be broad v7")
    if len(base["samples"]) != len(broad["samples"]):
        raise ValueError("v6 and v7 sample counts differ")
    ...
    for position, (base_row, broad_row) in enumerate(
        ...
        if broad_row["generation_rule"] != expected_rule:
            continue
        if (
            base_row["split"] != broad_row["split"]
            or base_row["split"] != "train"
            ...
            raise ValueError(
                "packing-isolation row changes split or family contract"
            )
        samples[position] = copy.deepcopy(broad_row)
        selected_positions.append(position)
        selected_rows.append(broad_row)
    if len(selected_rows) != 8:
        raise ValueError(
            f"packing-isolation expected 8 rows, got {len(selected_rows)}"
        )
    ...
    if any(overlaps.values()):
        raise ValueError(f"packing-isolation rows overlap v1-v6: {overlaps}")
    rendered_selected = _canonical_json(selected_rows)
    leaked_case_ids = [
        row["case_id"]
        for row in feedback["rows"]
    ...
    if leaked_case_ids:
        raise ValueError(
            "sealed case IDs leaked into packing-isolation data: "
            f"{leaked_case_ids[:5]}"
        )
    if (
        [row for row in samples if row["split"] == "validation"]
        ...
        raise ValueError("v9 must preserve all v6 development rows")

    dataset = {
        "schema_version": SCHEMA_VERSION,
        "dataset_id": "packing-isolation-preservation-mix-v9",
        "version": "v9",
        "source": {
            "source_kind": "deterministic_synthetic",
            "generator": "nano_data_pipeline.analog",
            ...
            "replacement_count": len(selected_rows),
            "replacement_family_counts": {
                "packing_efficiency_effective_volume": len(selected_rows)
            },
            "selected_positions_sha256": _hash(
                _canonical_json(selected_positions)
            ),
        ...
        "policy": {
            ...
            "contains_model_outputs": False,
            "contains_teacher_outputs": False,
            "purpose": "packing_family_isolation_sft_smoke",
            "observed_validation_reused": True,
            "validation_role": "development_gate_only",
            "all_numeric_targets_deterministically_verified": True,
            "all_intermediate_steps_verified": True,
...
def build_schedule_isolation_preservation_mix_dataset(
    feedback_manifest_path: Path,
    failure_family_receipt_path: Path,
    base_dataset_path: Path,
    broad_dataset_path: Path,
    ...
    if (
        receipt.get("schema_version")
        != "nano_harness_failure_family_receipt_v1"
        or "weighted_recurring_schedule_total"
        not in {row["family"] for row in receipt.get("families", [])}
        or receipt.get("policy", {}).get("contains_case_ids") is not False
        or receipt.get("policy", {}).get("contains_prompts") is not False
        or receipt.get("policy", {}).get("contains_references") is not False
        ...
        raise ValueError("failure-family receipt violates the v10 boundary")
    ...
    validate_analog_dataset(broad)
    if base.get("dataset_id") != "targeted-preservation-mix-v6":
        raise ValueError("schedule-isolation base must be v6")
    if broad.get("dataset_id") != "failure-targeted-preservation-mix-v7":
        raise ValueError("schedule-isolation source must be broad v7")
    if len(base["samples"]) != len(broad["samples"]):
        raise ValueError("v6 and v7 sample counts differ")
    ...
    for position, (base_row, broad_row) in enumerate(
        ...
        if (
            ...
            raise ValueError(
                "schedule-isolation row changes split or family contract"
            )
        samples[position] = copy.deepcopy(broad_row)
        selected_positions.append(position)
        selected_rows.append(broad_row)
    if len(selected_rows) != 8:
        raise ValueError(
            f"schedule-isolation expected 8 rows, got {len(selected_rows)}"
        )
    ...
    if any(overlaps.values()):
        raise ValueError(f"schedule-isolation rows overlap v1-v6: {overlaps}")
    rendered_selected = _canonical_json(selected_rows)
    leaked_case_ids = [
        row["case_id"]
        for row in feedback["rows"]
    ...
    if leaked_case_ids:
        raise ValueError(
            "sealed case IDs leaked into schedule-isolation data: "
            f"{leaked_case_ids[:5]}"
        )
    if (
        [row for row in samples if row["split"] == "validation"]
        ...
        raise ValueError("v10 must preserve all v6 development rows")

    dataset = {
        "schema_version": SCHEMA_VERSION,
        "dataset_id": "schedule-isolation-preservation-mix-v10",
        "version": "v10",
        "source": {
            "source_kind": "deterministic_synthetic",
            "generator": "nano_data_pipeline.analog",
            ...
            "replacement_count": len(selected_rows),
            "replacement_family_counts": {
                "weighted_recurring_schedule_total": len(selected_rows)
            },
            "selected_positions_sha256": _hash(
                _canonical_json(selected_positions)
            ),
        ...
        "policy": {
            ...
            "contains_model_outputs": False,
            "contains_teacher_outputs": False,
            "purpose": "schedule_family_isolation_sft_smoke",
            "observed_validation_reused": True,
            "validation_role": "development_gate_only",
            "all_numeric_targets_deterministically_verified": True,
            "all_intermediate_steps_verified": True,
